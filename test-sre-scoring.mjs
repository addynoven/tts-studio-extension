...
console.log('=== Scoring each formula ===\n');
let totalScore = 0;
for (let i = 0; i < mathEls.length; i++) {
  const el = mathEls[i];
  const latex = el.getAttribute('alttext') || '';
  ...
  totalScore += score;
  const keep = len <= 60 ? 'KEEP' : 'REPLACE with [formula]';
  console.log(`${i+1}. ${latex.slice(0, 60)}...`);
  console.log(`   SRE (${len} chars): "${speech.slice(0, 80)}${speech.length > 80 ? '...' : ''}"`);
  console.log(`   Score: ${score}/10 → ${keep}`);
  console.log('');
}
console.log(`Average score: ${(totalScore / mathEls.length).toFixed(1)}/10`);
