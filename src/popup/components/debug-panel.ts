 ...
 */
export function toggleDebugPanel(): boolean {
  isVisible = !isVisible;
  if (panelEl) {
...
export function addLog(entry: LogEntry): void {
  ...
  row.innerHTML = `
    ...
    <span style="color:${sourceColor[entry.source] || '#6b6b8f'};min-width:55px;flex-shrink:0;font-weight:600;">${entry.source}</span>
